...
const {get} = require('../routes/databaseBillRoute');
...
async function searchBills(req, res) {
  try {
    ...
    res.status(401).json({ error: "Lỗi hoặc không tìm thấy hóa đơn với số điện thoại này" });
  }
}
...
async function deleteBill(req, res) {
  try {
    ...
    if (result.deletedCount === 1){
      ...
      return res.status(401).json({ error: "Không tìm thấy hóa đơn với số điện thoại này" });
    }
  } catch (error) {
    console.error("Error delete medicin:", error);
    res.status(401).json({ error: "Xóa hóa đơn thất bại vui lòng kiểm tra lại" });
  }
}

module.exports = {
  deleteBill,getBills,searchBills
};