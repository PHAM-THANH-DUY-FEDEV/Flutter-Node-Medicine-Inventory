...
async function getStatis(req, res) {
  try {
    ...
    function checkExpiry(item) {
      if (item.ngayhethan) {
        const [expiryDay, expiryMonth, expiryYear] = item.ngayhethan
          .split("-")
          .map(Number);
        if (
          expiryYear < yy ||
    ...
    function checkNearExpiry(item) {
      if (item.ngaycanhbao) {
        const [expiryDay, expiryMonth, expiryYear] = item.ngaycanhbao
          .split("-")
          .map(Number);
        if (expiryYear == yy && expiryMonth == mm && dd >= expiryDay) {
